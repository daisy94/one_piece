(function ($) {
    ...
    BootstrapTable.prototype.initFixedColumns = function () {
        this.$fixedBody = $([
            '<div class="fixed-table-column" style="position: absolute; background-color: #fff; border-right:1px solid #c3c3c3; overflow:hidden; top:0; z-index:10;display:none;">',
            '<table>',
            '<thead></thead>',//��head����ʡ��
            '<tbody></tbody>',
            '</table>',
            '<table style="position:absolute; top:0; z-index:-5;">',//xjp20160627Ϊ�˶�����һ����ͷ
            '<thead></thead>',
            '</table>',
            '</div>'].join(''));
        ...
        this.$tableHeader.css({ 'position': 'relative', 'z-index': '5' });
        this.$el.css({ 'position': 'relative', 'z-index': '-20' });
        this.$tableBody.css({ 'position': 'relative', 'top': '-1px' });
        this.$fixedBody.find('table').eq(0).css({ 'position': 'relative', 'z-index': '-10' });
    ...
    BootstrapTable.prototype.fitBodyColumns = function () {
        ...
        this.$tableBody.on('scroll', function () {
            that.$fixedBody.find('table').eq(0).css({'top': -$(this).scrollTop()});
        });
