...
!function ($, moment) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        var DRPTemplate = '<div class="daterangepicker dropdown-menu">' +
                 '<div class="calendar left" style="float:left"></div>' +
                '<div class="calendar right" style="float:left"></div>' +                
                '<div class="clear"></div>' +
                '<div class="ranges">' +
                  '<div class="range_inputs">' +
                    ...
                    '</div>' +
                    '<button class="clearBtn btn btn-link" style="display:none;" ></button>&nbsp;' +                 
                    '<button class="cancelBtn"></button>&nbsp;' +
                     '<button class="applyBtn" disabled="disabled" style="margin:0 10px;"></button>&nbsp;' +
                  '</div>' +
                '</div>' +
              '</div>';
        ...
        updateInputText: function () {
            if (this.element.is('input') && !this.singleDatePicker) {
            	...
            	if(this.maxDate){
                	...
                	var d = new Date(maxDate._d);
                	var p=new Date(this.endDate._d);
                	var dateD,dateP,monthD,monthP;
                    if(d.getDate()<10){
                        dateD = '0'+ d.getDate();  //补齐
                    }else{
                    	dateD = d.getDate();
                    }
                    if(p.getDate()<10){
                        dateP = '0'+ p.getDate();  //补齐
                    }else{
                    	dateP = p.getDate();
                    }
                    if((d.getMonth()+1)<10){
                    	monthD = '0'+ (d.getMonth()+1);  //补齐
                    }else{
                    	monthD = d.getMonth()+1;
                    }
                    if((p.getMonth()+1)<10){
                    	monthP = '0'+ (p.getMonth()+1);  //补齐
                    }else{
                    	monthP = p.getMonth()+1;
                    }                    
                	d=d.getFullYear() + '' + monthD + '' + dateD + '' + d.getHours() + '' + d.getMinutes() + '' + d.getSeconds(); 
                	p=p.getFullYear() + '' + monthP + '' + dateP + '' + p.getHours() + '' + p.getMinutes() + '' + p.getSeconds();
                	if(parseInt(p)>parseInt(d)){
                		this.element.find('input').val(this.startDate.format(this.format) + this.separator + maxDate.format(this.format));
                	}else if(parseInt(p)<=parseInt(d)){
